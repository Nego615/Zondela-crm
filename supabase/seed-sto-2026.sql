...
-- ============================================================================
-- The contract as it stands, entered as data: the room categories, the rates
-- chart, the supplements and every clause from the signed PDF, word for word.
--
-- Rooms are split the way an operator reads them — Standard and Deluxe are
-- different products, photographed and described separately — so each category
-- gets its own block in the document with its own rates.
--
-- Run it once, after schema.sql. It is idempotent on the version's name, so
-- re-running replaces the contract rather than creating a second 2026.
--
-- Afterwards: open STO → Agreement versions → Open, add photographs on
-- Property Sections, attach the PDF on Official PDF, and activate it.
...
declare
  ...
  v_deluxe uuid;
begin
  -- Replace any earlier attempt at this season. Everything below cascades, and
  -- no send can exist yet on a contract nobody activated.
  delete from sto_agreement_versions where name = 'Zondela House STO Rate Contract 2026';

  insert into sto_agreement_versions (
    name, year, status, valid_from, valid_to, summary, intro, rate_basis, rates_note
  ) values (
  ...
  -- Room categories
  -- ------------------------------------------------------------------------
  insert into sto_version_property_sections
    (version_id, name, description, meal_plan_notes, seasonal_notes, sort_order)
  values (
    v_id,
    'Standard Rooms',
  ...
  returning id into v_deluxe;

  -- ------------------------------------------------------------------------
  -- Rates chart — from 1st Jan 2026 up to 31st Dec 2026
  -- ------------------------------------------------------------------------
  insert into sto_version_rates
    (version_id, section_id, season, room_type, pax, bb_price, hb_price, fb_price,
     max_occupancy, currency, sort_order)
  values
    (v_id, v_standard, '2026 season', 'Standard Single',     1, 130, 150, 170, 1, 'USD', 0),
  ...
  -- Terms & conditions
  -- ------------------------------------------------------------------------
  insert into sto_version_terms (version_id, title, body, sort_order)
  values
    (
      v_id,
      'Meal Plan',
      'Rates are quoted on bed & breakfast (BB), half board (HB) and full board (FB).

Additional meals may be arranged: lunch USD 20 per person and dinner USD 20 per person.',
      0
    ),
    (
      v_id,
...
NB: Zondela House booking system is automated, so all bookings without payments will be cancelled automatically as per the policy above.',
      4
    ),
    (
      v_id,
...
In the case of a no-show, a no-show fee of the rate applicable for the full stay, including any other government taxes or levies, will be levied against the Client.',
    ...
    (
      v_id,
      'Change of Rates',
      'The contracted rates in this agreement are subject to change without notice; however, every effort will be made to maintain them. In the event of a change in Government taxes or levies affecting the rates herein, Zondela House reserves the right to pass on any increases.',
      7
    ),
    (
      v_id,
      'Confidentiality',
      'The Tour Operator agrees that the contents of this agreement are strictly confidential to the undersigned, and that no part of this agreement in any form may be divulged to a third party.',
      8
    );
end
$$;
