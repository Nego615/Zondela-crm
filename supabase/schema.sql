...
create policy "sto_agreements_access" on sto_agreements for all
  using (can_access_company(company_id) or created_by = auth.uid())
  with check (can_access_company(company_id) or created_by = auth.uid());
...
create policy "sto_agreement_items_access" on sto_agreement_items for all
  using (can_access_agreement(agreement_id))
  with check (can_access_agreement(agreement_id));
...
-- SEED DATA
-- ============================================================================
-- None. The schema installs empty; add your services on the STO page's Rate
-- card tab, then build agreements from them.
