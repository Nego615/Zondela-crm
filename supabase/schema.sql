...
-- @@BEGIN:migrations/0005_sto_agreement_template.sql@@
-- Generated from supabase/migrations/0005_sto_agreement_template.sql by supabase/build-schema.mjs.
-- Edit that file, then run `npm run sync:schema`.
...
-- @@END:migrations/0005_sto_agreement_template.sql@@

-- @@BEGIN:migrations/0006_admin_only_logo.sql@@
-- Generated from supabase/migrations/0006_admin_only_logo.sql by supabase/build-schema.mjs.
-- Edit that file, then run `npm run sync:schema`.
...
-- @@END:migrations/0006_admin_only_logo.sql@@

-- @@BEGIN:migrations/0007_agreement_partner_mark.sql@@
-- Generated from supabase/migrations/0007_agreement_partner_mark.sql by supabase/build-schema.mjs.
-- Edit that file, then run `npm run sync:schema`.
...
-- @@END:migrations/0007_agreement_partner_mark.sql@@

-- @@BEGIN:migrations/0008_default_agreement_template.sql@@
-- Generated from supabase/migrations/0008_default_agreement_template.sql by supabase/build-schema.mjs.
-- Edit that file, then run `npm run sync:schema`.
...
-- @@END:migrations/0008_default_agreement_template.sql@@

-- @@BEGIN:migrations/0009_admin_may_delete_users.sql@@
-- Generated from supabase/migrations/0009_admin_may_delete_users.sql by supabase/build-schema.mjs.
-- Edit that file, then run `npm run sync:schema`.
...
-- @@END:migrations/0009_admin_may_delete_users.sql@@

-- @@BEGIN:migrations/0010_agreement_cover_image.sql@@
-- Generated from supabase/migrations/0010_agreement_cover_image.sql by supabase/build-schema.mjs.
-- Edit that file, then run `npm run sync:schema`.

-- ============================================================================
-- 0010 — A COVER IMAGE ACROSS THE TOP OF THE AGREEMENT
-- ============================================================================
-- The banner has carried both houses' marks since 0007, on white. It now
-- carries a photograph of the house above them as well: the first thing an
-- operator sees on opening the link is the property they are being asked to
-- sell, not a letterhead.
--
-- One image for every agreement, held on org_settings beside the logo, because
-- it is the house's identity rather than a season's. It is treated the way
-- 0006 treats the logo — a wrong cover on a signed contract is not a typo
-- someone fixes — so it sits behind the same permission:
--
--   * the upload lands in the `branding` bucket, whose writes 0006 already
--     moved onto settings.logo, so nothing changes in storage;
--   * guard_org_logo() now refuses this column to anyone without
--     settings.logo, exactly as it refuses logo_url.
--
-- A Manager still edits every other field on the letterhead.
-- ----------------------------------------------------------------------------

alter table org_settings add column if not exists cover_image_url text;

update permissions
   set description = 'Upload or remove the mark and the cover image that head every agreement. Admin only.'
 where key = 'settings.logo';
...
begin
  if (new.logo_url is distinct from old.logo_url
     ...
     and not has_permission('settings.logo') then
    raise exception 'Only an administrator can change the organisation logo or cover image'
      using errcode = '42501';
  end if;
  return new;
end;
...
-- THE PUBLIC PAGE
-- ----------------------------------------------------------------------------
-- Re-created whole, as in 0004, 0005 and 0007. The only change from 0007 is
-- `cover_image_url` on the org object — an operator opening the link has no
-- session, so the page sees nothing this function does not name.
-- ----------------------------------------------------------------------------
...
begin
  ...
  return jsonb_build_object(
    ...
    'org', jsonb_build_object(
      ...
      'website', v_org.website,
      'logo_url', v_org.logo_url,
      'cover_image_url', v_org.cover_image_url,
      'brand_color', v_org.brand_color,
      'accent_color', v_org.accent_color,
      'agreement_footer', v_org.agreement_footer,
      'signatory_name', v_org.signatory_name,
